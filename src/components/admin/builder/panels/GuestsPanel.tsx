...
import { deliveryState, DELIVERY_LABELS, guestMessage } from '@/lib/guest-delivery';
...
export default function GuestsPanel({ data, onChange, onPreview, previewGuestId }: Props) {
  const included = allowsService(data.config, 'guestManagement');
  const tables = allowsService(data.config, 'tableAssignment');
  ...
  const load = useCallback(() => {
    if (!included) { setGuests([]); return; }
    setLoading(true);
    fetch(`/api/guests?id=${data.id}`)
      .then(async response => { const body = await response.json(); if (!response.ok) throw new Error(body.error || 'No se pudieron cargar los invitados'); return body; })
      .then(value => setGuests(Array.isArray(value) ? value.map(enrich) : []))
      .catch(error => { setNotice(error.message); setGuests([]); })
      .finally(() => setLoading(false));
  }, [data.id, enrich, included]);
  useEffect(() => { load(); }, [load]);
  ...
  const whatsappText = (guest: Guest) => {
    return guestMessage(data.config?.whatsappTemplate,guest,linkFor(guest.publicId));
  };
  ...
  const exportCsv = () => {
    ...
    const header = ['Nombre', 'Teléfono', 'Grupo', 'Acceso', 'Mesa', 'Pases', 'Estado', 'Pases confirmados', 'Seguimiento', 'Recordatorios', 'Enlace personal'];
    const rows = guests.map(guest => [guest.name, guest.phone ?? '', guest.group ?? '', EVENT_LABEL[guest.eventAccess ?? 'both'], guest.tableNo ?? '', guest.passes, STATUS_META[guest.status].label, guest.confirmedPasses ?? '', DELIVERY_LABELS[deliveryState(guest)], guest.reminderCount ?? 0, linkFor(guest.publicId)]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      total: guests.length, sent: guests.filter(guest => deliveryState(guest) !== 'pending').length, confirmed: confirmed.length,
  ...
  const visible = useMemo(() => {
    ...
    return guests.filter(guest => {
      ...
      if (filter === 'sent') return deliveryState(guest) !== 'pending';
      if (filter === 'unsent') return deliveryState(guest) === 'pending';
      return filter === 'all' || guest.status === filter;
    });
  }, [guests, search, filter]);

  if (!included) return <p className="m-4 rounded-xl bg-amber-50 p-4 text-sm text-amber-900 font-outfit">Este paquete no incluye invitados personalizados. Puedes registrar el servicio como adicional en Configuración.</p>;

  return (
    <div className="space-y-4 p-4">
      {notice && <p role="status" className="rounded-xl bg-amber-50 p-3 text-xs text-amber-900">{notice}</p>}
      ...
      <details className="rounded-2xl border border-gray-100 bg-white p-3"><summary className="cursor-pointer text-xs font-semibold text-gray-600 font-outfit">Mensajes personalizados de WhatsApp</summary><div className="mt-3 space-y-3"><label className="block text-[10px] text-gray-500 font-outfit">Invitación<textarea rows={3} maxLength={1200} value={data.config?.whatsappTemplate ?? ''} onChange={event=>onChange?.({config:{...(data.config??{}),whatsappTemplate:event.target.value}})} placeholder="¡Hola {nombre}! Aquí tienes tu invitación: {link}" className="mt-1 w-full rounded-xl border border-gray-200 p-2 text-xs"/></label><label className="block text-[10px] text-gray-500 font-outfit">Recordatorio<textarea rows={3} maxLength={1200} value={data.config?.whatsappReminderTemplate ?? ''} onChange={event=>onChange?.({config:{...(data.config??{}),whatsappReminderTemplate:event.target.value}})} placeholder="Hola {nombre}, te recordamos confirmar aquí: {link}" className="mt-1 w-full rounded-xl border border-gray-200 p-2 text-xs"/></label><p className="text-[9px] leading-relaxed text-gray-400">Variables: {'{nombre}'}, {'{link}'}, {'{pases}'}, {'{mesa}'}, {'{codigo}'}. Sin proveedor externo no podemos confirmar envío, entrega o lectura.</p></div></details>
      ...
      {!guests.length ? <div className="rounded-xl bg-gray-50 p-6 text-center text-sm text-gray-400 font-outfit">Añade invitados para crear enlaces, contenido y pases personales.</div> : <div className="space-y-2">
        {visible.map(guest => <div key={guest.id} className={`space-y-2.5 rounded-2xl border bg-white p-3 transition-all ${previewGuestId === guest.id ? 'border-cyan-300 shadow-[0_8px_25px_rgba(8,145,178,.10)]' : 'border-gray-100'}`}>
          ...
          <div className="grid grid-cols-[72px_72px_1fr] gap-2"><label className="text-[9px] text-gray-400 font-outfit">Mesa<input disabled={!tables} title={tables ? "Mesa del invitado" : "No incluida en este paquete"} value={guest.tableNo ?? ''} onChange={event => patchGuest(guest.id, { tableNo: event.target.value })} className="mt-0.5 w-full rounded-lg border border-gray-200 px-2 py-1 text-center text-xs outline-none"/></label><label className="text-[9px] text-gray-400 font-outfit">Pases<input type="number" min={1} max={20} value={guest.passes} onChange={event => patchGuest(guest.id, { passes: Math.max(1, Math.min(20, parseInt(event.target.value) || 1)) })} className="mt-0.5 w-full rounded-lg border border-gray-200 px-2 py-1 text-center text-xs outline-none"/></label><label className="text-[9px] text-gray-400 font-outfit">Acceso<select value={guest.eventAccess ?? 'both'} onChange={event => patchGuest(guest.id, { eventAccess: event.target.value as GuestEventAccess })} className="mt-0.5 w-full rounded-lg border border-gray-200 px-1.5 py-1 text-[10px] outline-none"><option value="both">Todo</option><option value="ceremony">Ceremonia</option><option value="reception">Recepción</option></select></label></div>
          ...
          <div className="grid grid-cols-4 gap-1.5"><button type="button" onClick={() => onPreview?.(previewGuestId === guest.id ? null : guest)} className={`rounded-lg border py-1.5 text-[10px] font-outfit ${previewGuestId === guest.id ? 'border-cyan-300 bg-cyan-50 text-cyan-700' : 'border-gray-200 text-gray-500'}`}>{previewGuestId === guest.id ? '◎ Viendo' : 'Vista'}</button><button type="button" disabled={deliveryState(guest)!=='pending'} onClick={() => patchGuest(guest.id, { sent: true, deliveryStatus:'marked' })} className={`rounded-lg border py-1.5 text-[10px] font-outfit disabled:opacity-50 ${deliveryState(guest)!=='pending' ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-500'}`}>{DELIVERY_LABELS[deliveryState(guest)]}</button><button type="button" onClick={() => copy(linkFor(guest.publicId), guest.publicId)} className="rounded-lg border border-gray-200 py-1.5 text-[10px] text-gray-500 font-outfit">{copied === guest.publicId ? '✓ Copiado' : 'Copiar link'}</button><button type="button" onClick={() => setMessageGuestId(value => value === guest.id ? null : guest.id)} className="rounded-lg bg-[#25D366] py-1.5 text-[10px] text-white font-outfit">WhatsApp</button></div>
          {messageGuestId === guest.id && <div className="space-y-2 rounded-xl border border-emerald-100 bg-emerald-50/50 p-2.5"><p className="whitespace-pre-wrap text-[11px] leading-relaxed text-gray-600 font-outfit">{whatsappText(guest)}</p><div className="flex gap-2"><button type="button" onClick={() => copy(whatsappText(guest), `message-${guest.id}`)} className="flex-1 rounded-lg border border-emerald-200 bg-white py-1.5 text-[10px] text-emerald-700 font-outfit">{copied === `message-${guest.id}` ? '✓ Copiado' : 'Copiar mensaje'}</button><a href={waLink(guest)} target="_blank" rel="noopener noreferrer" className="flex-1 rounded-lg bg-[#25D366] py-1.5 text-center text-[10px] text-white font-outfit">Abrir WhatsApp</a></div><p className="text-[9px] text-emerald-800">Abrir no confirma envío ni entrega. Registra el seguimiento desde el panel del anfitrión.</p></div>}
