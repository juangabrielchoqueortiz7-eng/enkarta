...
import { collectPublicationResources, publicationMetrics } from '@/lib/publication-audit';
...
interface RemoteAudit {
  ...
  summary: { checked: number; failed: number; totalBytes: number; averageResponseMs: number; slow: number; heavy: number };
}
...
export default function PublicationAuditPanel({ data, validation, onOpenBlock }: Props) {
  ...
  const [remote, setRemote] = useState<RemoteAudit | null>(null);
  ...
  const remoteIssues = useMemo<BuilderIssue[]>(() => {
    ...
    remote.results.forEach(result => {
      ...
      else if ((result.bytes || 0) > 1_500_000) issues.push({ severity: 'warning', category: 'rendimiento', blockId: result.blockId, title: `${result.label} es pesado`, detail: `Pesa ${formatBytes(result.bytes || 0)}. Conviene mantener cada imagen por debajo de 1.5 MB.` });
  ...
  const runResources = async () => {
    ...
    try {
      const response = await fetch('/api/admin/publication-audit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ invitationId: data.id, resources }) });
      if (!response.ok) throw new Error('audit-failed');
      setRemote(await response.json() as RemoteAudit);
  ...
  return (
    <section className="space-y-4">
      <div className="overflow-hidden rounded-[22px] border border-[#e7e0ec] bg-white shadow-sm">
        <div className="bg-[linear-gradient(135deg,#252438_0%,#51445f_65%,#79607d_100%)] p-4 text-white">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1"><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/50 font-outfit">Auditor profesional</p><h3 className="mt-1 font-playfair text-xl">{score >= 90 ? 'Lista para impresionar' : score >= 70 ? 'Casi lista para publicar' : 'Necesita una última revisión'}</h3><p className="mt-1 text-[11px] leading-relaxed text-white/60 font-outfit">Contenido, accesibilidad, responsive, enlaces y rendimiento en una sola revisión.</p></div>
          </div>
      ...
      <div className="space-y-2">
        {!visibleIssues.length && <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-center"><span className="text-xl">✓</span><p className="mt-1 text-sm font-semibold text-emerald-800 font-outfit">Todo correcto en esta categoría</p></div>}
      ...
      <div className="rounded-2xl border border-cyan-100 bg-cyan-50/60 p-3">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          {([['cover', 'Portada'], ['middle', 'Sección media'], ['end', 'Cierre']] as const).map(([position, label]) => <a key={position} href={`/i/${data.slug}?preview=1&capture=${position}`} target="_blank" rel="noopener noreferrer" className="overflow-hidden rounded-xl border border-cyan-100 bg-white"><div className="relative h-24 overflow-hidden bg-[#ece7df]"><iframe title={`Captura ${label}`} src={`/i/${data.slug}?preview=1&capture=${position}`} tabIndex={-1} className="pointer-events-none absolute left-0 top-0 h-[680px] w-[390px] origin-top-left scale-[.25] border-0" /></div><p className="p-1.5 text-center text-[8px] font-semibold text-cyan-800 font-outfit">{label}</p></a>)}
        </div>
