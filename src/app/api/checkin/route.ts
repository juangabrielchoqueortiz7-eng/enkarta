...
import { canManageInvitation, getAdminSession, getHostSession, invitationIdOfAttendee } from '@/lib/host-session';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!admin && !host) return forbidden();
    ...
    if (!admin) query = query.eq('invitation_id', host!);
    ...
    if (!(await canManageInvitation(owner.invitation_id))) return forbidden();
    ...
    return privateJson({ guest: mapGuestRow(data.guest), attendees: data.attendees.map(mapAttendeeRow) });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc('enkarta_checkin', {
      ...
      p_expected_revision: body.expectedRevision, p_request_id: body.requestId, p_operator: admin ? 'admin' : 'host:' + host,
    });
    if (error) return serviceError(error);
